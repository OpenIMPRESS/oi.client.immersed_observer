...
public class SteamVR_ControllerManager : MonoBehaviour
{
    ...
    [Tooltip("For each object above, assign the serial of the tracker.")]
    public string[] assigned_tracker_serial;
    ...
    public bool isReserved(uint deviceIndex) {
        var system = OpenVR.System;
        if (system != null) {
            ETrackedPropertyError error = new ETrackedPropertyError();
            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            system.GetStringTrackedDeviceProperty(deviceIndex, ETrackedDeviceProperty.Prop_SerialNumber_String, sb, OpenVR.k_unMaxPropertyStringSize, ref error);
            var serial = sb.ToString();
            for (int i = 0; i < assigned_tracker_serial.Length; i++) {
                if (assigned_tracker_serial[i].Equals(serial)) {
                    return true;
                }
            }
        }
    ...
    public void Refresh() {
        ...
        System.Collections.Generic.List<int> preAssignedObjects = new System.Collections.Generic.List<int>();
        ...
        for (uint deviceIndex = 0; deviceIndex < connected.Length; deviceIndex++) {
            ...
            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            system.GetStringTrackedDeviceProperty(deviceIndex, ETrackedDeviceProperty.Prop_SerialNumber_String, sb, OpenVR.k_unMaxPropertyStringSize, ref error);
            var deviceClass = system.GetTrackedDeviceClass(deviceIndex);
            var probablyUniqueDeviceSerial = sb.ToString();

            if (probablyUniqueDeviceSerial.Length == 0 && !(deviceClass == ETrackedDeviceClass.Controller || deviceClass == ETrackedDeviceClass.GenericTracker))
                continue;

            for (int i = 0; i < assigned_tracker_serial.Length; i++) {
                if (assigned_tracker_serial[i].Equals(probablyUniqueDeviceSerial)) {
                    SetTrackedDeviceIndex(i + 2, deviceIndex);
                    preAssignedObjects.Add(i + 2);
                }
            }
        }
		...
		while (objectIndex < objects.Length) {
            if (preAssignedObjects.Contains(objectIndex)) {
                objectIndex++;
                continue;
            }
